...
export function parseStorySource(source: string): StoryData {
  ...
  for (const block of passageBlocks) {
    const match = block.match(/^(?:段落\s+"([^\"]+)"\s*[:：]|::\s*([^\n]+))\s*\n?(.*)$/s)
    if (!match) {
      continue
    }
...
function readBalancedBlock(source: string, startIndex: number, openChar: string, closeChar: string): BalancedBlock | null {
...
function consumeFnDefinition(source: string, startIndex: number): { name: string; code: string; endIndex: number } | null {
...
function extractAndRegisterFunctions(input: string, registry: Record<string, string>): string {
  let result = ''
  let cursor = 0
  let searchFrom = 0
...
function renderMarkdownInline(input: string): string {
  ...
  return segments
    .map((segment) => {
      ...
      working = working.replace(/\[([^\]]+)\]\(([^)]+)\)/g, (_full, label, href) => {
      ...
      working = working.replace(/__INLINE_CODE_(\d+)__/g, (_full, index: string) => codePlaceholders[Number(index)] ?? '')
      return working
    })
    .join('')
}
...
function replaceIfMacros(input: string, variables: VariableMap, story: StoryData, routeTo: (target: string) => void): string {
  let result = ''
  let cursor = 0
  let searchFrom = 0
...
export function applyPassageEntryEffects(content: string, variables: VariableMap): void {
...
function executeStoryJsFunction(name: string, args: unknown[], variables: VariableMap): unknown {
...
function parseCallArgs(argsSegment: string | undefined, variables: VariableMap): unknown[] {
...
function replaceCallExpressions(expression: string, variables: VariableMap): string {
  return expression.replace(CALL_IN_EXPRESSION_PATTERN, (_full: string, name: string, argsSegment?: string) => {
    const args = parseCallArgs(argsSegment, variables)
    return toExpressionLiteral(executeStoryJsFunction(name, args, variables))
  })
}

export function evaluateExpression(expression: string, variables: VariableMap): unknown {
...
export function evaluateCondition(condition: string, variables: VariableMap): boolean {
...
export function applyStoryAction(action: string, variables: VariableMap): void {
  ...
  const setMatch = cleaned.match(/^set:\s*(\$[A-Za-z_][A-Za-z0-9_]*)\s+to\s+(.+)$/i)
  if (setMatch) {
    const variableName = setMatch[1].slice(1)
...
function replaceTextWithHtml(raw: string, variables: VariableMap, story: StoryData, routeTo: (target: string) => void): string {
  ...
  working = working.replace(callPattern, (_full: string, name: string, argsRaw?: string) => {
    const args = parseCallArgs(argsRaw, variables)
...
export function renderStoryText(input: string, variables: VariableMap, story: StoryData, routeTo: (target: string) => void): string {
...
function sanitizeTag(tagText: string, tagName: string): string {
  ...
  for (const match of attributes.matchAll(attrRegex)) {
    ...
    const formattedValue = rawValue.startsWith('"') || rawValue.startsWith("'") ? rawValue.slice(1, -1) : rawValue
    safeAttributes.push(`${rawName}="${escapeHtml(formattedValue)}"`)
  }

  const attributeString = safeAttributes.length ? ` ${safeAttributes.join(' ')}` : ''
  return `<${tagName}${attributeString}>`
}

export function buildStandaloneExport(story: StoryData, variables: VariableMap, currentPassage: string): string {
