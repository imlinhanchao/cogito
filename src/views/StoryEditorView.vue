<template>
  <div class="story-shell h-full min-h-[calc(100vh-10rem)] p-4 lg:p-6">
    <div class="grid h-full gap-4 lg:grid-cols-[280px_minmax(0,1fr)]">
      <aside class="rounded-2xl border border-base-300 bg-base-100 p-3 shadow-sm">
        <div class="mb-3 flex items-center justify-between px-1">
          <h2 class="text-lg font-bold">段落列表</h2>
          <button class="btn btn-sm btn-primary" type="button" @click="addPassage">新增</button>
        </div>

        <div class="mb-2">
          <input v-model="searchFilter" placeholder="通过名称或 tag 搜索..." class="input input-sm w-full" />
        </div>

        <div class="space-y-2">
          <div v-for="passage in filteredPassages" :key="passage.name" class="flex items-center justify-between gap-2">
            <button
              type="button"
              class="flex-1 flex items-center justify-between rounded-xl border px-3 py-2 text-left transition"
              :class="selectedPassage === passage.name ? 'border-primary bg-primary/10' : 'border-base-300 bg-base-200 hover:border-primary/70'"
              @click="selectPassage(passage.name)"
            >
              <span class="truncate font-medium">{{ passage.name }}</span>
              <span class="badge badge-ghost badge-sm">{{ passage.tags.length || 0 }}</span>
            </button>
            <div class="flex items-center gap-2">
              <button class="btn btn-xs btn-ghost" type="button" @click.stop="copyPassageName(passage.name)" title="复制段落名">复制</button>
            </div>
          </div>
        </div>
      </aside>

      <main class="rounded-2xl border border-base-300 bg-base-100 p-4 shadow-sm">
        ...
        <div class="mb-4 flex flex-wrap gap-2">
          <button class="btn btn-sm btn-outline" type="button" @click="insertSnippet('[[' + selectedPassage + '|]]')">链接</button>
          <button class="btn btn-sm btn-outline" type="button" @click="insertSnippet('(if: $var > 0)[文本](else:)[文本]')">分支</button>
          <button class="btn btn-sm btn-outline" type="button" @click="insertSnippet('(set: $score to $score + 1)')">赋值</button>
          <button class="btn btn-sm btn-outline" type="button" @click="insertSnippet('(print: $score)')">打印</button>
          <button class="btn btn-sm btn-outline" type="button" @click="insertJsGlobalSnippet">全局 JS 定义</button>
          <button class="btn btn-sm btn-outline" type="button" @click="insertCallSnippet">调用 JS 函数</button>
        </div>
          ...
          <div class="rounded-xl border border-base-300 bg-base-200 p-3">
            <div class="mb-2 flex items-center justify-between">
              <div class="flex items-center gap-3">
                ...
                <span class="text-sm text-base-content/60">场景：{{ selectedPassage }}</span>
              </div>
              <div class="flex items-center gap-2">
                <button class="btn btn-xs btn-outline" type="button" @click="renamePassage">重命名</button>
                <button class="btn btn-xs btn-error btn-outline" type="button" @click="deletePassage">删除</button>
              </div>
            </div>
            <textarea
            ...
            <div class="mt-3 flex items-center gap-2">
              ...
              <input v-model="tagEditValue" class="input input-sm flex-1" placeholder="用逗号分隔标签" />
              <button class="btn btn-xs btn-primary" type="button" @click="saveTags">保存</button>
            </div>
          </div>

          <div class="space-y-4 rounded-xl border border-base-300 bg-base-200 p-3">
            <div class="rounded-xl bg-base-100 p-3">
              <p class="mb-2 text-sm font-semibold uppercase tracking-[0.24em] text-base-content/70">故事信息</p>
              <ul class="space-y-2 text-sm text-base-content/80">
                <li>标题: <strong>{{ story.title || '未命名故事' }}</strong></li>
                <li>起始段落: <strong>{{ story.startPassage || selectedPassage }}</strong></li>
                <li>段落数: <strong>{{ story.passages.length }}</strong></li>
              </ul>
            </div>

            <div class="rounded-xl bg-base-100 p-3">
              <p class="mb-2 text-sm font-semibold uppercase tracking-[0.24em] text-base-content/70">变量面板</p>
              <div class="space-y-2 text-sm">
                <div v-if="Object.keys(variables).length === 0" class="text-base-content/60">暂无变量</div>
                <div v-for="(value, key) in variables" :key="key" class="flex items-center justify-between gap-2 rounded-lg bg-base-200 px-2 py-1">
                  <span>{{ key }}</span>
                  <strong>{{ value }}</strong>
                </div>
              </div>
        ...
        <section class="mt-6 rounded-2xl border border-base-300 bg-base-100 p-4 shadow-sm">
          <div class="mb-4 flex items-center justify-between gap-3">
            <div>
              <p class="text-xs font-semibold uppercase tracking-[0.28em] text-base-content/50">Syntax Manual</p>
...
''粗体'' //斜体// ~~删除线~~ ^^上标^^ ,,下标,,
            ...
            <article class="rounded-2xl border border-base-300 bg-base-200 p-4 lg:col-span-2">
...
(print: $newScore)
                ...
                <p>函数由 <strong>new Function('vars','args', code)</strong> 执行，接收 <strong>vars</strong>（变量字典引用）和 <strong>args</strong>（参数数组）。函数可以读取与修改 <strong>vars</strong>，并通过 <code>return</code> 返回值，可用 <strong>(set: $x to (call:"name" ...))</strong> 将返回值赋给变量。</p>
    ...
    <dialog ref="dialogRef" class="modal">
      <div class="modal-box">
        ...
        <div class="modal-action">
          ...
          <button class="btn btn-primary" type="button" @click="confirmRun">确认</button>
        </div>
      </div>
...
const selectedPassageContent = computed({
  get: () => {
    const current = story.value.passages.find((passage) => passage.name === selectedPassage.value) ?? story.value.passages[0]
...
const ensurePassage = (name: string) => {
  ...
  if (!story.value.passages.some((passage) => passage.name === normalized)) {
    story.value.passages.push({ name: normalized, tags: [], content: '新段落内容' })
  }
...
const renamePassage = () => {
  const current = story.value.passages.find((passage) => passage.name === selectedPassage.value)
  if (!current) {
    return
  }
...
const deletePassage = () => {
  ...
  story.value.passages = story.value.passages.filter((passage) => passage.name !== selectedPassage.value)
...
const saveTags = () => {
  const p = story.value.passages.find((x) => x.name === selectedPassage.value)
  if (!p) return
  p.tags = (tagEditValue.value || '').split(',').map((s) => s.trim()).filter(Boolean)
}
