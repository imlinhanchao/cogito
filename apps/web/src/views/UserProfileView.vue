<template>
  <div v-if="userInfo" class="p-4 sm:p-6 max-w-7xl w-full mx-auto space-y-6">
    <div
      class="flex items-center gap-4 bg-base-100 p-4 rounded-2xl border border-base-200/80"
    >
      <div class="avatar">
        <div
          v-if="!userInfo.avatar"
          class="w-16 h-16 rounded-full bg-primary/10 flex items-center justify-center text-xl font-bold text-primary"
        >
      ...
      <div class="ml-auto">
        <button class="btn btn-sm btn-error btn-soft" @click="logout">
          退出登录
        </button>
      </div>
    </div>
    ...
    <div v-else>
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <div
          v-for="s in stories"
          :key="s.id"
          class="card bg-base-100 border border-base-200/80 p-3 rounded-xl"
        >
          <div class="flex flex-col h-full">
            ...
            <div
              class="mt-3 flex items-center justify-between text-xs text-base-content/60"
            >
      ...
      <div v-if="!stories.length" class="text-center text-base-content/60 py-8">
        该用户尚未发布任何故事。
      </div>
    </div>
  </div>
