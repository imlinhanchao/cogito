...
NestJS backend service using TypeScript, TypeORM with MySQL, JWT, and Swagger.

### Setup

Copy `.env.example` to `.env`, set the MySQL credentials, and set a strong `JWT_SECRET`.
...
Error responses use the same keys. `stack` is populated only outside production.
