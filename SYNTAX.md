...
- 支持将动作附加到链接上，常见形式：
  ...
  - `(link:"查看钥匙")[(display: "Key")]`：点击后在链接原位置嵌入 `Key` 段落，并替换链接本身。
  - `(link:"点击我")[(set: $x to 2)]`：点击时执行赋值。
  - `[[文本]](call:"name" arg1 arg2)`：点击时调用已定义的 JS 函数 `name`，参数可为字符串、数字或 `$var` 变量引用。

说明要点：
- 附着在链接后的 `(set:)` / `(call:)` / `(point:)` 等动作不会在段落渲染（进入时）执行，而只在用户点击该链接时执行。这能保证“点击时发生的变化”不会误在进入段落时触发。请在需要立即生效的赋值放在段落正文里 `(set:)`，而需要绑定到某个选项才运行的动作附着到链接后。
