...
说明：支持比较操作：`>`、`<`、`>=`、`<=`，以及 `is`、`is not`、`eq`、`ne` 等比较关键字。
...
文件生成于项目根目录，用于在编辑器下方显示的语法说明的独立文档版本。若需扩展此文档，请在此基础上编辑并提交。

---

## JS 函数（扩展）

编辑器支持在故事中定义并调用轻量的 JS 函数，这在需要复杂计算或对 `vars` 进行自定义更新时很有用。

定义格式：

```text
(fn:"name")[<javascript code>]
```
