...
段落是故事的基本单位，每个段落以 `:: 段落名` 开头，后续内容为段落正文。例如：
...
> 注意：条件后必须添加方括号，括号与条件表达式之间至少要有一个空格，例如 `(if: $health > 5)[ [[分数为零|GameOver]] ]`。

## 成就/结局标记

- `(point: 名称|描述)`：段落入口（或点击时，若附着在链接）会将标记入队列，渲染时以徽章显示在渲染顶部。适用于提示玩家获得物品或小成就。
- `(end: 名称|描述)`：标记结局，仅当段落不含任何可点击链接且该段仅包含一个 `(end:)` 时生效，渲染时在页面底部显示结局徽章。若期望通过点击触发结局，请将结局放入独立段落并由链接导航或 `(display:)` 展示。
...
> [!NOTE]
> 附着在链接(link)后的 `(set:)` / `(call:)` / `(point:)` 等动作不会在进入段落时执行，而只在用户点击该链接时执行；把需要立即生效的赋值放在段落正文里 `(set:)`，把需要绑定到某个选项才运行的动作附着到链接后。
